...
uses
  ...
  frxClass, frxExportPDF, frxDBSet, uniBasicGrid, uniDBGrid, frxBarcode,
  uniMemo;
...
uses
  ...
  UnitReportDisplay, UnitFOHomeService, UnitQz, System.NetEncoding, UnitJasper;
...
var
  I, baru_detail_id, header_id: integer;
...
var
  pf1_nolab, pf2_nolab, nolab_length, nolab_max, reset_nolab, last_nolab, xnow,
    pf1, pf2, nomor, kode, ada_subtest, pf1_NoNota, pf2_NoNota, NoNota_length,
    NoNota_max, reset_nonota, last_nonota, nomor_nota: string;
...
begin
  ...
  begin
    ...
    try
      ...
      QSave.SQL.Text :=
        'insert into order_header(tanggal,pasien_id,dokter_id,tipe_pasien_id,tipe_hasil_id,nomor,sub_total,tagihan,disc_pct,disc_amount,total,flgLunas,cetak,urgency_id,ket_tgl_selesai) '
        + ' values(now(),:pasien_id,:dokter_id,:tipe_pasien_id,:tipe_hasil_id,:nomor,:sub_total,:tagihan,:disc_pct,:disc_amount,:total,:flgLunas,:cetak,:urgency_id,:ket_tgl_selesai )';
...
begin
  try
    ...
    // ambil string zpl
    if Qzpl.Active then Qzpl.Active := False;
    Qzpl.SQL.Text := 'SELECT value_text FROM parameters WHERE name = '+QuotedStr('ZPL_LABEL_ORDER');
    Qzpl.Active := True;
