...
begin
  ...
  begin
    ...
    QOrder.SQL.Clear;
    QOrder.SQL.Text :=
      'SELECT oh.id, oh.dokter_id, d.nama dokter, oh.sub_total, oh.disc_pct, oh.disc_amount,oh.total,oh.tgl_ambil_hasil,oh.catatan '
      + ' FROM order_header oh ' + ' LEFT JOIN dokter d ON oh.dokter_id = d.id '
      + ' WHERE oh.id = :id ';
